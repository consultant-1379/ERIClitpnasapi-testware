...
public class LITPnasapiTestRunner extends TorTestCaseHelper {
    
    Logger logger = Logger.getLogger(LITPnasapiTestRunner.class);
    ...
    @TestId(id = "CXP9031141-1", title = "Upgrade ERIClitpnasapi rpm on the MS")
    @Test(groups={"ACCEPTANCE"})
    public void upgradeERIClitpnasapiRPM() throws TimeoutException, FileNotFoundException {

        rpmUpgradeOperator.initialise("nasapi");
     ...
     * @PRIORITY HIGH
     */
    @TestId(id = "CXP9031141-2", title = "Run python test cases for ERIClitpnasapi package")
    @Test(groups={"CDB_REGRESSION", "ACCEPTANCE"})
    public void runERIClitpnasapiTests() {
     ...
     * @PRE Execution of test {@link #runERIClitpnasapiTests()}
     * @PRIORITY HIGH
     * @param className
     * @param name
     * @param failures
     */
    @TestId(id = "CXP9031141-3", title = "Parse xml outputs from python tests")
